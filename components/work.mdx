...
`Home.js` maintains a `currentWorkExperience` array. It maps over that array to render one `Work` component per entry, and initially displays only the first two entries:

```jsx
{currentWorkExperience.slice(0, 2).map((job) => (
  <Work
    key={job.title}
    title={job.title}
    text={job.text}
...
<Tip>
  To show all experience entries, remove the `.slice(0, 2)` call or increase the slice limit.
</Tip>
