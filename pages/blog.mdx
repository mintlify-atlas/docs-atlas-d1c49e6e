...
The blog section consists of two parts: a listing page (`src/Blog.js`) and individual article pages routed via `src/components/ViewBlog.js`.
...
```js
// src/constants/articles/netflix.js (example)
export default {
    title: string,   // Article headline
    date: string,    // Publication date
    content: any     // Full article content (rendered by ViewBlog)
}
```
...
<Steps>
  <Step title="Create a new article file">
    ...
    const myNewArticle = {
        ...
        date: 'March 2026',
        content: (
            // JSX or string content for the article body
            <div>
...
The `Blog` component (`src/Blog.js`) imports the `articles` array and Patrick's face photo, suggesting a listing UI was planned. Implementing it would involve mapping over `articles` and rendering each entry as a link to `/blog/:id`:
